  ...
  try {
    ...
    const activeContents =
      geminiContents.length > 0
        ? geminiContents
        : [{ role: 'user', parts: [{ text: 'Hello, let us begin the panel interview.' }] }];

    // Primary model is gemini-3.1-flash-lite as requested
      ...
      const readableStream = new ReadableStream({
        async start(controller) {
          try {
            ...
            for (const key of keys) {
              ...
              for (const modelName of modelsToTry) {
                try {
                  ...
                  console.warn(`[Interview Agent LLM Stream] Key ...${key.slice(-4)} ${modelName} failed:`, err);
                }
              }
              if (streamStarted) break;
            }
            ...
            console.error('[Interview Agent LLM Stream Error]:', err);
    ...
    for (const key of keys) {
      ...
      for (const modelName of modelsToTry) {
        try {
          ...
          lastError = err;
          console.warn(`[Interview Agent LLM] Key ...${key.slice(-4)} ${modelName} failed:`, err);
        }
      }
      if (outputText) break;
    }
    ...
    console.error('Interview Agent LLM Error:', error);
    return NextResponse.json(
