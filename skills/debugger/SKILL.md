...
- If the context is **unclear** → use `ask_question` with the four domains above as options. Don't guess.
...
**Agentic execution in Antigravity IDE:** Run probes directly using environment tools — execute terminal commands (`run_command`), run test suites, grep codebase logs, or deploy `browser_subagent` for visual/DOM debugging — rather than making the user paste output manually. Only ask the user when an external remote environment or physical device is needed.
...
### Phase 8 — Debrief (Required "Root Cause & Fix")
After completing any bug fix, always summarize the resolution in this exact format:
...
- **When in doubt, ask** — `ask_question` is cheap; a wrong domain wastes the user's time.
