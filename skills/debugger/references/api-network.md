...
### Edge Functions, Serverless & Streaming / Realtime (`edge-realtime`)

Modern edge runtimes (Supabase Edge Functions, Cloudflare Workers, Next.js Server Actions) and realtime streams (WebSockets, SSE, WebRTC) have unique failure modes:

1. **Edge Function Headers & CORS**: Deno/V8 edge runtimes require explicit CORS headers on every response, including the `OPTIONS` method:
   ```typescript
   if (req.method === 'OPTIONS') {
     return new Response('ok', { headers: corsHeaders });
   }
   ```
2. **Server-Sent Events (SSE) / Streaming**: If client receives nothing until the connection closes, the reverse proxy or middleware is buffering the response. Ensure `Content-Type: text/event-stream`, `Cache-Control: no-cache`, and `Connection: keep-alive` are sent immediately, and flush chunks as they arrive.
3. **WebRTC / Agora Voice & Audio Drops**: Inspect ICE connection states (`iceConnectionState === 'disconnected' | 'failed'`). Verify token expiration timestamps and channel name matching between peer tokens.
4. **WebSocket Connection Races**: When messages sent immediately after `new WebSocket()` vanish, check if the client waited for `ws.onopen` before calling `ws.send()`.
