...
Generic AI output shares recognizable failure patterns. Run your design through this filter before delivery:

- ❌ **No purple-on-black generic cards**: Do not default to `#8b5cf6` or `#a855f7` on dark slate unless specifically requested. Use curated palettes (warm sand, deep navy, forest emerald, brutalist monochrome).
- ❌ **No identical 3-column feature grids**: Vary visual weight. Make one feature prominent with a rich interactive preview or larger footprint.
- ❌ **No floating decorative icons without context**: Every icon must serve a functional purpose or anchor a specific piece of data.
- ❌ **No low-contrast illegible text**: Ensure text on glassmorphic cards maintains strict 4.5:1 contrast against dynamic backgrounds.
- ✅ **Tactile Micro-Interactions**: Buttons must have active press states (`transform: scale(0.98)`), cards subtle hover elevation, and focus rings with adequate offset.

---

## 12. Visual Verification Protocol (Antigravity Browser Testing)

Whenever building or redesigning UI in Antigravity:

1. **Exercise in Browser**: Use `browser_subagent` to load the local dev server or rendered HTML.
2. **Multi-Viewport Check**: Verify layout across:
   - **Mobile** (375px / 390px width) — test drawer navigation, touch target spacing, and sticky CTAs.
   - **Tablet** (768px width) — test grid wraps and responsive typography.
   - **Desktop** (1440px width) — test maximum content widths and horizontal rhythm.
3. **Capture Proof**: Record or capture visual confirmation of the rendered UI to guarantee zero layout shifts or visual clipping before declaring done.
