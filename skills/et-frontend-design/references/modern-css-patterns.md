...
## @starting-style & Popover Transitions

Animate elements entering the DOM or transitioning from `display: none` without JavaScript:
...
Align nested items across sibling cards perfectly along the parent grid tracks:
...
.card {
  ...
  grid-row: span 4; /* header, media, body, actions all line up perfectly across cards */
  gap: var(--space-4);
}
```
