...
- **Reasoning Models (Thinking Tokens)** — For frontier models with native thinking (Gemini 2.5/3, Claude 3.7 Thinking, OpenAI o1/o3-mini), avoid redundant "think step by step" blocks or forced internal scratchpads that conflict with native reasoning. Guide the *evaluation criteria*, *invariants*, and *stop conditions*, and let native reasoning do the deduction.
...
- **Decomposition** — Too big for one shot? Recommend a plan/spec step, or chain-of-thought for non-reasoning models. In an agent workflow this often means splitting one mega-prompt into several nodes/steps.
...
- **Output contract & Structured Outputs** — Is the desired format explicit? Prefer strict JSON Schema / Function Calling contracts over markdown prose. Specify field names, types, allowed enums, and **what to emit when a value is absent** (usually `null` — not omitted, not invented).

### D. Priming & Context Caching
- **Context Caching Architecture** — Place static system instructions, tool definitions, and reference documentation at the very top of the prompt to maximize prefix-cache hits across requests. Keep dynamic variables and variable payload data at the bottom.
...
- **Rule awareness** — Does the draft contradict established rules (a project's `GEMINI.md`/`AGENTS.md`/`ai-context.md`, a known design system, an API's output schema)? Flag the contradiction now — it costs a wasted run later.
